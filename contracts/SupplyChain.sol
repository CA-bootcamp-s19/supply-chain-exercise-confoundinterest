...
contract SupplyChain {
  ...
  */
  ...
  modifier forSale (uint sku) {
    ...
    require(item.state == uint(State.ForSale), "only an item in the for sale state can be purchased");
    require(item.buyer == address(0), "only an item that has not already been bought can be purchased");
    _;
  }
  modifier sold (uint sku) {
    require(items[sku].state == uint(State.Sold));
    _;
  }
  modifier shipped (uint sku) {
    require(items[sku].state == uint(State.Shipped));
    _;
  }
  modifier received (uint sku) {
    require(items[sku].state == uint(State.Received));
    _;
  }
